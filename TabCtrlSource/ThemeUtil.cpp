...
CThemeUtil::CThemeUtil()
{
	...
	if(IsWinXP())
	{
//		m_hUxThemeDll = AfxLoadLibrary(CString(_T("UxTheme.dll"))); //Canceled by zhang ziye,2007,09.10
...
void CThemeUtil::FreeLibrary()
{
	...
	{
		//AfxFreeLibrary(m_hUxThemeDll); //Canceled by zhang ziye,2007,
...
BOOL CThemeUtil::OpenThemeData(HWND hWnd, LPCWSTR pszClassList)
{
	...
	if(pfnIsThemeActive && pfnIsThemeActive())
	{
		...
		if(m_hTheme)
		{
			...
			{
				//if((m_hRcDll = AfxLoadLibrary(CString(szThemeFileName))))//Canceled by zhang ziye,2007,09.10
...
void CThemeUtil::CloseThemeData()
{
	...
	if(m_hRcDll!=NULL)
	{
		//AfxFreeLibrary(m_hRcDll);//Canceled by zhang ziye,2007,09.10
